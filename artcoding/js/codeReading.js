...
window.onload = function () {
	$.ajax({
		...
		success: function (data) {
			var len = data.length;
			for (var i = 0; i < len; i++) {
				var commentDetail = data[i];
				console.log(comment); //打印所有评论
				...
				oDiv.className = "nicheng";
				oDiv.innerHTML = commentDetail.name;

				oBox.appendChild(oDiv);

				var oDiv = document.createElement("div");
				oDiv.className = "pinglun";
				oDiv.innerHTML = commentDetail.comment;
				oBox.appendChild(oDiv);

				var oDiv = document.createElement("div");
				oDiv.className = "shijian";
				if (commentDetail.uid == commentDetail.nuid) {
					oDiv.innerHTML = commentDetail.date + "<a href='javascript:;' name=" + commentDetail.cid + ">删除</a> <hr>";
				} else {
					oDiv.innerHTML = commentDetail.date + "<hr>";
				}
